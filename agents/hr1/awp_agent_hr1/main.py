"""hr1 process entrypoint: `python -m awp_agent_hr1.main` — a bus-worker
...
async def _main() -> None:
    ...
    mcp = MCP(
        {
            ...
            "hrsourcing": os.environ["MCP_HRSOURCING_URL"],
            "docs": os.environ["MCP_DOCS_URL"],
            "approvals": os.environ["MCP_APPROVALS_URL"],
            "comms": os.environ["MCP_COMMS_URL"],
        },
        principal_jwt_provider=lambda: mint_service_jwt(AgentId.HR1.value, SCOPES),
    )
    ...
    async def mirror_status(env: TaskEnvelope, result: TaskResult) -> None:
        # Safety net for graph-level crashes HR-1's own `respond` node
        # never reaches — see AgentApp.__init__'s `on_result` docstring.
        # Also the *only* path that ever persists `result.summary` — no
        # node anywhere calls `update_task` on the happy path either, so
        # `get_task_status` returned `result: None` for every completed
        # task until this included it (DEVIATIONS.md; same gap
        # `agents/ops1/awp_agent_ops1/main.py` fixed in Sprint 10 and
        # flagged as "almost certainly exists" everywhere else).
        await mcp.call(
            "erp",
            "update_task",
            {
    ...
    app = AgentApp(AgentId.HR1, graph, checkpoints, graph_name="hr1", on_result=mirror_status)

    start_metrics_server()  # doc 10 HLD C19: Prometheus scrapes this agent's :9100/metrics

    await app.run_forever(bus, consumer_name=os.environ.get("HOSTNAME", "hr1-worker-1"))
